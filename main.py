...
def run_customer_churn_results(final_model, dataset: dict, raw_csv_data: pd.DataFrame):
    ...
    #predictions = final_model.model.predict(dataset["x_test_norm"], verbose=0)
    predictions = final_model.model.predict(final_model.x_test_norm, verbose=0)

    first_line = "Row | Customer ID | Probability | Status"
    results = [first_line, "-" * len(first_line)]
    for idx, probability in enumerate(predictions):
        ...
        results.append(line)
        #results.append(f"{idx} Customer ID: {customer_id} : Probability: {prob[0]:.4f} -> {status}")
...
def run_customer_churn_results_orig(models: list, best_model_name: str, dataset: dict):
    ...
    for i, prob in enumerate(predictions[:10]): # Showing first 10 for clarity
        is_churning = prob[0] > CUSTOMER_CHURN_PROB_THRESHOLD
...
def run_main_pipeline(args: dict):
    ...
    print(f"final results type = {type(best_model_name)}")
...
def main():
    ...
    # Generate Smote Classification Report
    show_banner(sgd_smote_model.title, 'Classification Report')
    y_predictor = (sgd_smote_model.predict(data_sp.testing['normalized']) > CUSTOMER_CHURN_PROB_THRESHOLD).astype(int)
